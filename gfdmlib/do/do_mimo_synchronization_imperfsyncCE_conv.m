function [ preamble_starts, cfo_values, rx_less_cfo, mean_cfo_hat ] = do_mimo_synchronization_imperfsyncCE_conv( config, rx_signal, mode, option )
...
if strcmp(mode,'primary')
   ...
   Np = config.Np+config.Ncp;
   N = config.N;
   distance = Np;
   
   length_init_zeropad = 400;
   
   corr_stop = length_init_zeropad+2*Np+N;
...
%    crosscorr = prod(crosscorr_iT,2);
   ...
   cfo_flag = 1;
    if cfo_flag
        corr_in_cfo = corr(length_init_zeropad-100+1:end); 
        coarse_starts = ref_sync_peakfinding(distance, N,corr_in_cfo);     
        coarse_starts = coarse_starts + length_init_zeropad -100;
        if length(coarse_starts) > 1
           coarse_starts = coarse_starts(1); 
        end
...
%     rx_signal = ifft(fft(rx_signal).*(repmat(sto_shifts,[1 nR]).^sto));
   ...
   combined1 = crosscorr(1:length(corr)) / Np;%corr .* crosscorr(1:length(corr)) / Np;
...
%    hold on; plot(abs(crosscorr)./max(abs(crosscorr))); legend('combined','autocorr','crosscorr')
   combined1 = combined1(length_init_zeropad-100+1:end); 
   preamble_starts = ref_sync_peakfinding_relatedwork(distance*2, N ,combined1);
   preamble_starts = preamble_starts + length_init_zeropad -100;
...
elseif strcmp(mode,'secondary')  
    ...
    if nargin > 3
        if strcmp(option,'circUW')
            distance = 2*N;
            cfo_mode = 'secondaryCirc';
            xp_iT = config.preamble.xp_iT;
            xp_iT = [xp_iT(end/2+1:end,:); circshift(xp_iT(end/2+1:end,:),[0 2])];            
        else
            distance = N; cfo_mode = 'secondary';
        end
    else
        distance = N; cfo_mode = 'secondary';
    end
...
%     corr = ref_sync_autocorrelate(distance,rx_signal, Ncp, Ncs,0,'manual',frame_length + config.preamble.N_preamble/2);
    ...
    cfo_flag = 1;
    if cfo_flag
        coarse_starts = ref_sync_peakfinding(distance, frame_length,corr);
        
        cfo_values_iR = zeros(length(coarse_starts),nR);
        for iR = 1:nR
            cfo_values_iR(:,iR) = ref_frac_cfo_estimation_uw(coarse_starts, corr_iR(:,iR), N, Np,cfo_mode);
        end
        cfo_values = mean(cfo_values_iR,2);
...
%     ylabel('$M_{i_{\rm R}, {\rm CC}}^{\rm (Pr)}[n]$','interpreter','latex')
    ...
    combined = crosscorr(1:length(corr)) / distance;%corr .* crosscorr(1:length(corr)) / distance;
...
%     [];
   if 0
    ...
    if_loss_use_previous_sync_flag = 0;
    if if_loss_use_previous_sync_flag
        ...
        while sync_counts < noblk
            ...
            missed_blk =  find(blk_distances>(mean(blk_distances)+std(blk_distances)));
            
            correct_blk_distances = blk_distances;
            correct_blk_distances(missed_blk) = [];
            for i_missed = 1:length(missed_blk)
                if i_missed > 1
                    missed_blk(i_missed) = missed_blk(i_missed)+1;
                end
                preamble_starts = [preamble_starts(1:missed_blk(i_missed)) preamble_starts(missed_blk(i_missed))+floor(mean(correct_blk_distances)) preamble_starts(missed_blk(i_missed)+1:end)];
            end
            sync_counts = length(preamble_starts);
        end
