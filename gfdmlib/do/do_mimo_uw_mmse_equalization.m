function [ d_hat , R_dd_tilde, equalizer, R_dd_hat, HA] = do_mimo_uw_mmse_equalization( config, snr, wH_lmmse_in, R_hh_err, Y_all_v)
...
% Xp_iT = config.preamble.Xp_uw;
    ...
    for iR = 1:nR
        HA_iR = [];
        for iT = 1:nT
            HA_iR = [HA_iR (repmat(wH_lmmse_in(:,iT,iR),[1 N]).*FA)];
        end
        HA = [HA; HA_iR];
    end
    HA(off_bins,:) = [];
