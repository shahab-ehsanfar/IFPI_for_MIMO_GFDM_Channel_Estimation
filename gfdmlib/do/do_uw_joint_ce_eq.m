function [ d_hat, h_hat_out, Rx_matrices ] = do_uw_joint_ce_eq(config, snr, y, h_hat_primary, R_hh_err_primary, noblk, wiener_flag)
    ...
    if (wiener_flag == 1 || (wiener_flag == 2 && blk < B))
        ...
        for iR = 1:nR
            for iT = 1:nT
                ...
                [h_all_blks(:,iT,iR,:), Covariance(:,:,iT,iR,:)] = do_wiener_smoodiction_uw(h_hat_wiener_in, R_hh_err_2nd_iT, size(h_hat_wiener_in,2), N, n_H, n_Hd, Fs, fd, numTaps);
            end
        end
