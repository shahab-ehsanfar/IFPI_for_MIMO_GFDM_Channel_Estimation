function [ vH_lmmse_out, d_hat, Heq, R_hh_err ] = do_CE_MMSE_PIC(est_type, p, conf, pilots_conf, no_PIC_iterations, snr, vH_lmmse_in, R_hh_err, Dd1_indices, ycp, Y_all_v, dhat_genie, vH_genie)
...
genie_aided_flag = 0;
if genie_aided_flag
   total_no_of_iterations = no_PIC_iterations+2;
else
   total_no_of_iterations = no_PIC_iterations;
end

for its=1:total_no_of_iterations
...
%     end
    ...
    %    Dd_tilde(:,:,2) = reshape(sigma_dhat2,[p.K p.M]);
        ...
        %R_PsiPsi_new1 = JFHFIcpA*blkdiag(diag(Dd1_indices),diag(diag(R_dd_tilde(1:N,1:N))),diag(Dd1_indices),diag(diag(R_dd_tilde(N+1:end,N+1:end))))*JFHFIcpA';
