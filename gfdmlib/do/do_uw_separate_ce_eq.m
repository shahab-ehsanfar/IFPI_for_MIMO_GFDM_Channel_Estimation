function [ d_hat, h_hat_out, Rx_matrices ] = do_uw_separate_ce_eq(config, snr, y, h_hat_primary, R_hh_err_primary, noblk, wiener_flag)
...
for blk = 1:noblk  
    ...
    % Channel MMSE Equalization
    [ d_hat{blk} , ~, ~, R_dd_hat] = do_mimo_uw_equalization( config, snr, H_hat_in, R_hh_err_in, Y_all);
    ...
    if (wiener_flag == 1 || (wiener_flag == 2 && blk < B))
        ...
        for iR = 1:nR
            for iT = 1:nT
                ...
                [h_all_blks(:,iT,iR,:), Covariance(:,:,iT,iR,:)] = do_wiener_smoodiction_uw(h_hat_wiener_in, R_hh_err_2nd_iT, size(h_hat_wiener_in,2), N, n_H, Fs, fd, numTaps);
            end
        end
        h_hat_in = h_all_blks(:,:,:,end);
        
        h_hat_all = cat(4, h_hat_all, h_hat_in);
        ...
        h_hat_out{blk+1} = h_hat_in;
    elseif (wiener_flag == 2 && blk >= B)
        h_all_blks = zeros(numTaps,nT,nR,blk+2);
        Covariance = zeros(numTaps,numTaps,nT,nR,blk+2);
        for iR = 1:nR
            for iT = 1:nT
                h_hat_wiener_in = cat(2, squeeze(h_hat_all(:,iT,iR,:)), h_hat_secondary(:,iT,iR));
                R_hh_err_2nd_iT = R_hh_err_2nd((iT-1)*numTaps+1:iT*numTaps,(iT-1)*numTaps+1:iT*numTaps,iR);
                [h_all_blks(:,iT,iR,:), Covariance(:,:,iT,iR,:)] = do_wiener_smoodiction_uw(h_hat_wiener_in, R_hh_err_2nd_iT, size(h_hat_wiener_in,2)+1, N, n_H, Fs, fd, numTaps);
            end
        end
        h_hat_in = h_all_blks(:,:,:,end);
