function [ preamble_starts, cfo_values, rx_less_cfo, mean_cfo_hat ] = do_mimo_synchronization( config, rx_signal, mode, option )
...
%    crosscorr = prod(crosscorr_iT,2);
   ...
   cfo_flag = 1;
    if cfo_flag
        coarse_starts = ref_sync_peakfinding(distance, N,corr); 
%         coarse_starts = 101;
        ...
        for iR = 1:nR
%             coarse_starts = ref_sync_peakfinding(distance, N,corr_iR(:,iR)); coarse_starts = coarse_starts(1);
...
%         preamble = [xp_iT(end/2+1:end,iT); zeros(N_data,1); xp_iT(1:end/2,iT)];
        ...
        crosscorr_iT(:,(iR-1)*nT+iT) = ref_sync_crosscorrelation(xp_iT(:,iT),rx_less_cfo(:,iR),'manual',N,noUW);                 
    end
    end
    crosscorr = sum(abs(crosscorr_iT),2);
...
%     figure; plot(abs(crosscorr(1:length(corr))))
%     figure; plot(abs(combined))
%     
    preamble_starts = ref_sync_peakfinding(distance, frame_length,combined);
    if noUW == 3
        preamble_starts = [preamble_starts preamble_starts(end)+N];
    elseif noUW == 4
        preamble_starts = [preamble_starts preamble_starts(end)+N preamble_starts(end)+2*N]; 
    elseif noUW == 8
        preamble_starts = [preamble_starts preamble_starts(end)+N preamble_starts(end)+2*N preamble_starts(end)+3*N preamble_starts(end)+4*N preamble_starts(end)+5*N preamble_starts(end)+6*N];
    end
