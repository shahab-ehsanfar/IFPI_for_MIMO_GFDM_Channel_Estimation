function [ d_hat , R_dd_tilde] = do_CEPIC_equalization( conf, pilots_conf, snr, wH_lmmse_in, R_hh_err, Y_all_v)
...
m = 4;
    ...
    for iR = 1:nR
        HA_iR = [];
        for iT = 1:nT
            HA_iR = [HA_iR ZF*(repmat(wH_lmmse_in(:,iT+(iR-1)*nR),[1 N]).*FA)];
        end
        HA = [HA; HA_iR];
    end
   % HA(off_bins,:) = [];
 ...
 %   R_psidp_i = zeros(N,N,nR);
    for iR = 0:nR-1
        ...
        for iT = 0:nT-1
            ...
            R_psipsi_i(:,:,iT+1) = ZF*(FA_Rdd_AF_tx1 .* R_HHe_i(:,:,iT+1))*ZF';
...
%    R_psipsi_tot = kron(eye(nR),sum(R_psipsi_i(on_bins_single,on_bins_single,:),3));
    ...
    R_dy = repmat(diag(R_dd),[1 1056]) .* HA';
    ...
    R_yy = mat1 + mat2 + mat3 + varN*eye(1056);
    ...
    for iR = 1:nR
        HXp_iR = 0;
        for iT = 1:nT
            HXp_iR = HXp_iR + ZF*(wH_lmmse_in(:,iT+(iR-1)*nR) .* Xp_iT(:,iT));
        end
        HXp_mat(:,iR) = HXp_iR;
    end
    HXp = HXp_mat(:);
...
%     R_dd_tilde = R_dd - equalizer*R_dy';
    if pilots_conf.blk == 1
        d_hat = equalizer*(kron(eye(nT),ZF)*Y_all_v);
    else
        d_hat = equalizer*(kron(eye(nT),ZF)*Y_all_v - HXp );
    end
    R_dd_tilde = R_dd(:,:,m) - equalizer*R_dy';
