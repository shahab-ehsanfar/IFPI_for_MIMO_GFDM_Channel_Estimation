function [ bitErr_gen, ser, mi ] = do_detection(p,N,nT,b,bc,dd,Y,Heq,snr,PCCC,code_interleaver, bicm_interleaver,Ainv_F_mimo,F_mimo_A,p2)
...
if nargin < 15 || p2.M ~= 1    
    ...
    [C_genie_0, ~] = calc_softdec_equichan(p2,Ainv_F_mimo,F_mimo_A,QHeq,Q,nT);   
    for iT = 1:nT
        n0 = (iT-1)*N;
        nd0 = (iT-1)*N_d;
        C_genie_mat = reshape(C_genie_0(n0+1:n0+N),p.M,p.K).';
        C_genie(nd0+1:nd0+N_d) = do_unmap_p(p,C_genie_mat).';
    end
    E_h = ones(size(C_genie));    
    EsNo = 1;
  %  EsNo = 10^(snr/10);
end
...
for iT = 1:nT
   ...
   elseif p2.M == 1
       Dhat(:,:,iT) = Xhat(N*(iT-1)+1:iT*N); %do_demodulate(p2, ifft_u(Xhat(N*(iT-1)+1:iT*N)),'ZF');
...
%   [bh(:,iT), bit_llr(:,iT)] = do_decode_pccc_loopedDemod(dhat(:,iT), b(:,iT).', EsNo, H_equiv(n0+1:n0+N_d), PCCC, code_interleaver(iT,:), bicm_interleaver(iT,:));
 
   
%     E_h = ones(size(E_h));
%     n0 = (iT-1)*N_d;
%     EsNo = 10^(snr/10);
%    [bh2(:,iT), bit_llr2(:,iT)] = do_decode_pccc(dhat(:,iT), b(:,iT).', EsNo, E_h(n0+1:n0+N_d), PCCC, code_interleaver(iT,:), bicm_interleaver(iT,:));
    
end

mi = mutualinfo(bit_llr(:),bc(:));
 
% mi2 = mutualinfo(bit_llr2(:),bc(:));
...
% mi = 0;
% D = do_map_p(p,dd(:,1));
% s = do_qamdemodulate(D(2,:),p.mu);
% Dhat = do_map_p(p,dhat(:,1));
% sh = do_qamdemodulate(Dhat(2,:),p.mu);
