function [ Hhat ] = do_Chan_Estimation_ofdm( p, Y, Dp, numPaths, method, snr, R_HH, R_XdXd, H, Dd, Fp, Delta_k)
%
% method:
...
%         H = Fp*ifft(H0);
        ...
        % Apply the a-priori-Knowledge of number of paths to the estimation
        h_ls = (Delta_k/p.Mp)*Fp'*(H_ls)/sqrt(N); 
        h_ls(numPaths+1:end) = 0;
        %Hhat = Fp*(h_ls)*sqrt(N); 
        Hhat = h_ls;   
    ...
    case 11
        ...
        VarS = 1; % Signal power
        VarN = VarS/(10^(snr/10));  
        S = VarN*eye(p.Mp*N/Delta_k); %Fp*(VarN*eye(N))*Fp'/(N/Delta_k);
        ...
        % Apply the a-priori-Knowledge of number of paths to the estimation
        h_lmmse = (Delta_k/p.Mp)*Fp'*(H_lmmse)/sqrt(N);
        h_lmmse(numPaths+1:end) = 0;
