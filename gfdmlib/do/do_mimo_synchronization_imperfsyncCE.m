function [ preamble_starts, cfo_values, rx_less_cfo, mean_cfo_hat ] = do_mimo_synchronization_imperfsyncCE( config, rx_signal, mode, option )
...
%    hold on; plot(abs(crosscorr)./max(abs(crosscorr))); legend('combined','autocorr','crosscorr')
   ...
   preamble_starts = ref_sync_peakfinding(distance, N ,combined1);
...
%     figure; plot(abs(combined)./max(abs(combined+1)));
    ...
    preamble_starts = ref_sync_peakfinding(distance, frame_length,combined);
%     [];
   if 0
