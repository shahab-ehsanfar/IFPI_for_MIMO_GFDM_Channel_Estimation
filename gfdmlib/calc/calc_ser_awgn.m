...
function ser = calc_ser_awgn(p, recType, snr, channel, mse)
...
for i=1:fftLen
    if strcmp(recType, 'ZF')
        for si = 1:length(snr)
            H = (abs(fft(channel, fftLen)) - sqrt(mse(si))*(ones(fftLen,1))).^2;
            effSNR(si) = snr(si) + 10 * log10((H(i))/noiseEnhancement);
        end
    elseif strcmp(recType, 'MF')
        effSNR = -10* log10(1/H(i)*nLin+sigmaI);
    else
