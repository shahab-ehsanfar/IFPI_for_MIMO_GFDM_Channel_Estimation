...
function ser = calc_ser_awgn2(p, recType, snr, channel, mse)
...
for k=0:p.K-1
    ...
    NEF = zeros(length(snr),1);
    NEF2 = zeros(length(snr),1);
    for si = 1:length(snr)
        NEF(si) = sum(abs(Gs).^2./((abs(H) - sqrt(mse(si)) ).^2) );
        NEF2(si) = NEF(si) /(p.K*p.M);        
    end
    effSNR = snr + 10*log10(1./NEF2);
