function preamble_starts = ref_sync_peakfinding_relatedwork(distance, frame_length, metric)

% Adaptive threshold 
no_of_thresholds = floor(length(metric)/frame_length);
T= zeros(no_of_thresholds,1);
for i= 1:no_of_thresholds
    T(i) = abs(ref_sync_estimateThreshold(metric(frame_length*(i-1)+1:frame_length*i),1.5));
end
