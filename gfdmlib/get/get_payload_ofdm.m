...
%             Dd(floor(p.K/2),1,iT,blk) = sqrt(Np);
    ...
    [on_bins, on_bins1, off_bins, on_bins_single] = get_on_bins_ofdm(config.payload.p, nT,nR);
    ...
    % insert CP
    xd_iT_cp = zeros(N_data+Ncp,nT,noblk);
    for blk = 1:noblk 
        xd_iT_cp(:,:,blk) = [xd_iT(end-Ncp+1:end,:,blk); xd_iT(:,:,blk)];
    end
